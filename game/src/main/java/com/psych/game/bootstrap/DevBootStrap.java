...
public class DevBootStrap implements ApplicationListener<ContextRefreshedEvent> {
	...
	public void onApplicationEvent(ContextRefreshedEvent event) {
		...
		Player player = new Player.Builder().setAlias("surajAwasthi").setEmail("suawasthi@gmail.com").setPicUrl(
		...
		pl.add(player);
		Game game = Game.builder().players(pl).leader(player).gameStatus(GameStatus.PlAYER_jOINING).gameMode(GameMode.MOVIE_BUFF).noOfGames(34).hasEallen(false).build();
		gameRepo.save(game);
	}
